<template>
  <article class="rolesPage">
    <div class="background"></div>
    <div class="linkContainer">
      <template v-for="(gender, index) in ['male', 'female']" :key="index">
        <template
          v-for="(role, index) in ['elf', 'mage', 'royal', 'knight', 'darkElf']"
          :key="index"
        >
          <RouterLink
            :to="`/${gender}/${role}`"
            :class="`${gender} ${role}`"
            @click.stop="storeAudio.clickToPlayAudio('UI/audio_click.mp3')"
          >
          </RouterLink>
        </template>
      </template>
    ...
    <audio
      :src="storeAudio.getUrlForHashWhenProd('login/rolesPage_audio.mp3')"
      autoplay
      loop
    ></audio>
  </article>
