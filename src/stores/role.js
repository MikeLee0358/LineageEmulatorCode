...
export const useRoleStore = defineStore('role', () => {
    const role = reactive({
        data,
        gifUrl: computed(() => `${role.currentRole}/${role.currentRole}_${role.currentGender}.gif`),
        updatedAttr: 0,
    ...
    const getGenderClass = () => {
        if (role.currentGender === 'male') return `${role.currentRole} ${role.currentGender}`
        else if (role.currentGender === 'female') return `${role.currentRole} ${role.currentGender}`
    }
    const calcEquipAttribute = (string, equip) => {
        ...
        if (equip.isAttrEquip) {
            ...
            const plusAttr = computed(() => (role.currentData.basic[attr] += equip.attribute[attr]))
