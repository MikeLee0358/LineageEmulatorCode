...
export const useAlgorithmStore = defineStore('algorithm', () => {
    ...
    const getDiceValue = () => (dice.value = Number((Math.random() * 100).toFixed(2)))
    ...
    const getDiceSuccessValue = () => {
        const specialCases = () => {
            ...
            return (
                ...
                (storeScroll.isScrollType('cursed') && target.value >= Math.abs(target.safetyValue))
            )
        }
        const isValueOver9 = () => Math.abs(target.value) >= 9
        const isValueUnder4 = () => Math.abs(target.value) < 4
        const isValueUnderSafetyValue = () => Math.abs(target.value) < Math.abs(target.safetyValue)
        const getWeaponSucessValue = () => {
            /* Weapon Formula */
            //|underSafetyValue|:100%
            //|0~8|:33%
            //|9up|:10%
            if (isValueUnderSafetyValue()) return (dice.successValue = 100 + dice.bonus)
    ...
    const getRandomStateOneTo = (num) => (dice.state = Number(Math.floor(Math.random() * num) + 1))
    ...
    const updataChatAndValueOver9 = (value) => {
        ...
        if (storeScroll.isScrollType('white') && isSuccessIn([1, 2])) {
            ...
            dice.state = 1
            storeKnight.getGameChatEvent('weaponSuccess')
        } else if (storeScroll.isScrollType('blessed') && isSuccessIn([1, 2, 3, 4])) {
            target.value++
            dice.state = 1
            storeKnight.getGameChatEvent('weaponSuccess')
        } else {
