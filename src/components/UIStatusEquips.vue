<template>
  <ul class="uiStatusEquips">
    <li
      ...
      :style="{
        backgroundImage: `url(${storeRole.getUrlForHashWhenProd(equip.src)})`,
      }"
...
const changeCursor = () => {
  if (storeScroll.targetScroll === null)
    return `url(${storeRole.getUrlForHashWhenProd(
      "UI/UI_pointer.webp"
    )}), auto`;
  else
    return `url(${storeRole.getUrlForHashWhenProd("UI/UI_target.webp")}), auto`;
};
const getEquipInfo = (equip) => {
  const showPlusOrMinus = (value) => (value >= 0 ? `+${value}` : value);

  const getName = () => {
    const nameArmor = () => `${showPlusOrMinus(equip.value)} ${
      equip.name
    } (使用中)
...
const getDataForAlgorithm = (equip, event) => {
  ...
  const updateEquipValue = () =>
    setTimeout(() => (equip.value = storeAlgorithm.target.value), 0);
